...
public class Example2 {
	...
	public static def main(Array[String]) {
		...
		Scheduler.addPair(1,4); // dummy race!
		Scheduler.addPair(3,4);
		...
		finish{
			...
			async clocked(c){
				...
				print("1-2");
				delay();
				thread.wait("if(x == 6) raceValue = 3;", 3);
				if (x == 6) raceValue = 3;
				print("1-3");
				delay();
				thread.wait();
			...
			async clocked(c1){
				...
				thread.wait("if (raceValue == 3) raceValue = 2;", 4);
				if (raceValue == 3) raceValue = 2;
				print("2-2");
				delay();
				thread.wait("if (y == 4) y = 2;", 5);
				if (y == 4) y = 2;
				print("2-3");
				delay();
				thread.wait();
